...
export class CdkVinculacaoProcessoGridFilterComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.form.get('processoVinculado').valueChanges.subscribe(value => {
            if (value !== null) {
                if (typeof value === 'object' && value) {
                    this.filters = {
                        ...this.filters,
                        'processoVinculado.id': `eq:${value.id}`
                    };
                } else {
                    if (this.filters.hasOwnProperty('processoVinculado.id')) {
                        delete this.filters['processoVinculado.id'];
                    }
                }
            }
        });

        this.form.get('modalidadeVinculacaoProcesso').valueChanges.subscribe(value => {
            if (value !== null) {
                if (typeof value === 'object' && value) {
                    this.filters = {
                        ...this.filters,
                        'modalidadeVinculacaoProcesso.id': `eq:${value.id}`
                    };
                } else {
                    if (this.filters.hasOwnProperty('modalidadeVinculacaoProcesso.id')) {
                        delete this.filters['modalidadeVinculacaoProcesso.id'];
                    }
                }
            }
        });
    ...
    buscar(): void {
        this.emite();
        this._cdkSidebarService.getSidebar('cdk-vinculacao-processo-main-sidebar').close();
    }

...
export class CdkAssuntoGridFilterComponent implements OnInit {
        ...
        this.form.get('assuntoAdministrativo').valueChanges.subscribe(value => {
            if (value !== null) {
                if (typeof value === 'object' && value) {
                    this.filters = {
                        ...this.filters,
                        'assuntoAdministrativo.id': `eq:${value.id}`
                    };
                } else {
                    if (this.filters.hasOwnProperty('assuntoAdministrativo.id')) {
                        delete this.filters['assuntoAdministrativo.id'];
                    }
                }
            }
        });
    ...
    buscar(): void {
        this.emite();
        this._cdkSidebarService.getSidebar('cdk-assunto-main-sidebar').close();
    }

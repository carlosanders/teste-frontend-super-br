                ...
                mergeMap(action => this._vinculacaoEtiquetaService.save(action.payload).pipe(
                        mergeMap((response: VinculacaoEtiqueta) => [
                            ...
                            new OperacoesActions.Resultado({
                                ...
                                content: `Etiqueta na processo id ${action.payload.processo.id} criada com sucesso!`,
                                success: true,
                        ...
                        catchError((err) => {
                            ...
                            this._store.dispatch(new OperacoesActions.Resultado({
                                type: 'vinculacao_etiqueta',
                                content: `Houve erro no etiqueta na processo id ${action.payload.processo.id}! ${err.error.message}`,
                                success: false,
                                dateTime: moment()
                            }));
